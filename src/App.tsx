...
export default function App() {
  ...
  const renderSlide = () => {
    switch (currentSlide) {
      ...
      case 7: return <SlideCompare />;
      case 8: return <Slide7Quiz />;
      case 9: return <Slide8Summary />;
  ...
  return (
    <SoundContext.Provider value={{ play, soundEnabled, toggleSound }}>
      <div 
        ref={containerRef} 
        className="w-full h-dvh bg-slate-950 flex flex-col overflow-hidden font-sans"
      >
        <div className="flex-1 relative overflow-hidden" style={{ paddingBottom: 'min(6rem, 10vh)' }}>
        ...
        >
          <div className="flex items-center justify-between px-6 py-3" style={{ height: 'min(6rem, 10vh)' }}>
            <div className="flex items-center gap-3 w-1/4">
              <button
                onClick={toggleNavbar}
                className="p-2 rounded-full hover:bg-slate-800 transition-colors text-cyan-400"
              ...
              <span className="text-slate-400 text-base hidden lg:block">← → 切换页面</span>
            </div>
              ...
              {Array.from({ length: TOTAL_SLIDES }).map((_, i) => (
                <button
                  ...
                  className={`h-4 rounded-full transition-all duration-300 ${
                    currentSlide === i 
                      ? 'bg-cyan-500 w-14 shadow-[0_0_15px_rgba(34,211,238,0.8)]' 
            ...
            <div className="flex items-center justify-end gap-2 w-1/4">
              <div className="text-right mr-3 hidden lg:block">
                <p className="text-cyan-400 font-bold text-lg">{slideTitles[currentSlide]}</p>
                <p className="text-slate-500 text-base">{currentSlide + 1} / {TOTAL_SLIDES}</p>
              </div>
              ...
              <button
                ...
                className="p-2 rounded-full hover:bg-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-slate-300"
                aria-label="下一页"
              >
        ...
        {isNavbarCollapsed && (
          <button
            onClick={toggleNavbar}
            className="fixed right-0 bottom-0 p-3 bg-slate-900 border border-slate-700 rounded-l-xl hover:bg-slate-800 transition-colors text-cyan-400 shadow-lg z-30"
            title="展开导航栏"
          >
            <ChevronLeft size={24} />
          </button>
