...
const initialItems: DraggableItem[] = [
  { 
    ...
    reason: '虽然年龄增长身高也增长（相关联），但身高÷年龄不是固定值，比值不固定',
    zone: 'unclassified' 
  },
  { 
    id: '2', 
    text: '一本书已读页数和未读页数', 
    isProportional: false,
    reason: '虽然相关联，但已读页数÷未读页数不是固定值，比值不固定',
    zone: 'unclassified' 
  },
  { 
    id: '3', 
  ...
  { 
    ...
    text: '圆的周长和直径', 
    isProportional: true,
    reason: '周长÷直径=π（一定），两个条件都满足：相关联、比值一定',
    zone: 'unclassified' 
  },
];
...
const conditions = [
  ...
  { id: 2, key: 'ratioFixed', title: '比值一定', detail: '相对应的两个数的比值是固定不变的数' },
];
              ...
              {!showDefinition ? (
                ...
                >
                  <div className="flex items-center justify-center gap-2 mb-3 text-cyan-400">
                    <LockOpen size={24} /> <span className="text-lg font-bold tracking-widest">规律已解锁</span>
                  </div>
                  <p className="mb-3">
              ...
              >
                ...
                {showConditionsButton && !showConditions && (
                  ...
                  >
                    ...
                    >
                      <CheckSquare size={24} /> 点击查看判断正比例的两个必要条件
                    </motion.button>
                  </motion.div>
                )}
              </motion.div>
            ...
            >
              ...
              >
                <h3 className="text-lg font-black text-yellow-400 mb-2 flex items-center justify-center gap-2">
                  <CheckSquare size={20} /> 判断正比例的两个必要条件
                </h3>
                ...
                <div className="text-sm text-center mb-2 text-yellow-100 font-medium bg-yellow-900/30 p-2 rounded-lg border border-yellow-600/30">
                  ⚠️ 两个条件必须<strong className="text-yellow-300">同时成立</strong>
                </div>
