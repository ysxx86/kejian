    ...
    <div className="flex flex-col h-full bg-slate-950 p-6 md:p-12 text-slate-100 relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-5xl mx-auto w-full z-10">
        <AnimatePresence mode="wait">
          <motion.div key={currentQ} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="w-full bg-slate-900/90 backdrop-blur-md p-10 md:p-14 rounded-3xl shadow-2xl border-t-8 border-purple-500 border-x border-b border-slate-700">
            ...
            <AnimatePresence>
              {showExplanation && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-8">
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
      ...
      {showExplanation && (
        <div className="fixed bottom-28 right-8 z-50 flex flex-col items-end gap-4">
          <div className={clsx("p-6 rounded-2xl text-xl font-medium shadow-2xl border-2 max-w-md", 
            ...
            <p className="mb-2 font-bold text-2xl flex items-center gap-2">
              {(q.type === 'choice' && selectedOption === q.options?.find(o => o.isCorrect)?.id) || (q.type === 'fill' && isFillCorrect) || isTableChoiceCorrect
                ? <><CheckCircle2 size={28} className="text-emerald-400"/> 回答正确！+{q.points}积分 🎉</> 
                : <><XCircle size={28} className="text-red-400"/> 回答错误！❌</>}
            </p>
            <p className="leading-relaxed text-slate-300 text-lg">{q.explanation}</p>
          </div>
          <button onClick={handleNext} className="bg-gradient-to-r from-cyan-600 to-blue-600 text-white px-10 py-5 rounded-full text-2xl font-bold hover:from-cyan-500 hover:to-blue-500 transition-all shadow-2xl flex items-center gap-3 border border-cyan-400">
            {currentQ < questions.length - 1 ? "下一题" : "查看成绩"}
